...
export function IonicLab({ active, collectionOpen, onCollectionOpenChange, onCountChange }: Props) {
  ...
  function inspect(next: Graph) {
    ...
    for (const group of components(next)) {
      ...
      saveGroup(group);
      const key = `${match.compound.id}:${group.atoms.map(a => a.id).sort().join(',')}`;
      if (!seen.current.has(key)) { seen.current.add(key); setCelebration(match.compound); }
    }
  }
  ...
  function resummon(entry: IonicCompound) {
    ...
    seen.current.add(`${entry.id}:${group.atoms.map(a => a.id).sort().join(',')}`);
